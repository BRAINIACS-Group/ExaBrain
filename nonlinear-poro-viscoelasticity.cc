    ...
    class Time
    {
        public:
          ...
          double get_delta_t() const
          {
              double dt = delta_t;
              if (time_end_load > 0) {
            	 // if (time_current <= 0.75*time_end_load)
            	 //	  dt = 0.25*time_end_load;
...
// PerTaskData_ASM stores local contributions
    ...
    template <int dim> class BrainNanoFlatPunchBase : public Solid<dim>
    {
    	...
    	private:
    		virtual void make_grid()
    		{
    			// Not so good cylinder...
    			// Define center coordinates, radius and height of cylindrical geometry
    			const Point<dim-1> bottom_center(0.0, 0.0);
    			const double radius = 4.0;
    			const double height = 3.5;
    			const double indenter_radius = 0.5;
    			const Point<dim> top_center(0.0, 0.0, height);

    			// Create circular triangulation and extrude it into a cylinder
    			Triangulation<dim-1> triangulation_in;
    			GridGenerator::hyper_ball(triangulation_in, bottom_center, radius);
    			GridGenerator::extrude_triangulation(triangulation_in, 2, height, this->triangulation, true);

    			// Create and assign a cylindrical manifold to the triangulation
    			const CylindricalManifold<dim> cylinder_3d(2);
    			const types::manifold_id cylinder_id = 0;
    			// Assign a manifold object, here the CylindricalManifold "cylinder_3d", to a certain part of the triangulation,
    			// here the part with manifold_id "cylinder_id = 0" which are the boundary faces.
    			this->triangulation.set_manifold(cylinder_id, cylinder_3d);

    			// Apply global geometry scaling and refinement
    			GridTools::scale(this->parameters.scale, this->triangulation);
    			this->triangulation.refine_global(std::max(1U, this->parameters.global_refinement));
    			...
    			for (auto cell : this->triangulation.active_cell_iterators()) {
    				for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face) {
    					if (cell->face(face)->at_boundary() == true) {
    						if (cell->face(face)->center()[2] == 0.0)
    							cell->face(face)->set_boundary_id(1); //bottom
    						else if (cell->face(face)->center()[2] == height && top_center.distance(cell->face(face)->center()) < indenter_radius)
    							cell->face(face)->set_boundary_id(2); //loaded top
    						else if (cell->face(face)->center()[2] == height)
    							cell->face(face)->set_boundary_id(3); //unloaded top
    						else {
    							cell->face(face)->set_boundary_id(0); //cylinder hull
    							cell->face(face)->set_all_manifold_ids(cylinder_id);
    						}
    					}
    				}
    		...
    		virtual void define_tracked_vertices(std::vector<Point<dim>> &tracked_vertices)
    		{
    			tracked_vertices[0][0] = 0.0*this->parameters.scale;
    			tracked_vertices[0][1] = 0.0*this->parameters.scale;
    			tracked_vertices[0][2] = 3.5*this->parameters.scale; // TODO: use height instead of 3.5 or even top_center directly
    		...
    		virtual void make_dirichlet_constraints(AffineConstraints<double> &constraints)
    		{
    			// Top (unloaded) surface and cylinder hull are drained
    			if (this->time.get_timestep() < 2) {
    				VectorTools::interpolate_boundary_values(
    				    	this->dof_handler_ref,
    						0,
    			...
    			if (this->parameters.load_type == "displacement") {
    			    ...
    			    // Fix in x-y-plane under indenter tip
