...
namespace io {
        ...
        enum : ignore_column {ignore_no_column = 0,
...
private:
    ...
    void
    write_element_helper(std::ostream &element_out, const unsigned int row, const ArrayType& arr, unsigned int &count)
    {
        if (row < arr.size())
        {
            ++count;
    ...
    template <class ArrayHead, class...ArrayTail>
    bool
    write_row_helper (const unsigned int row, const ArrayHead& head, const ArrayTail&...tail)
    {
        static_assert (column_count == (1 + sizeof...(ArrayTail)),"");
        ...
        using expander = int[];
        (void)expander{0, (void(write_element_helper(row_out << ", ",row,tail,count)), 0)...};
...
// We select the type of problem to solve and introduce the desired load values.
      ...
      void Geometry::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.enter_subsection("testing_device@[type=porous_tension_compression_testing_device,instance=1]");
		{
          ...
          prm.declare_entry("Lateral drained", "drained",
        		  	  	    Patterns::Selection("drained|undrained"),
        		  	  	    "Set lateral surfaces to be drained or undrained.");

          prm.declare_entry("Bottom drained", "undrained",
                  		  	Patterns::Selection("drained|undrained"),
                  		  	"Set bottom surface to be drained or undrained.");

          prm.declare_entry("Lateral confined", "unconfined",
...
// Here we set the timestep size $ \varDelta t $ and the simulation end-time->
      ...
      void Time::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Time");
        {
          ...
          prm.declare_entry("End load time", "-1.0",
                            Patterns::Double(),
                            "End load time");
...
// We can choose the frequency of the data for the output files.
      ...
      void OutputParam::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Output parameters");
        {
          ...
          prm.declare_entry("Eigenvalue analysis", "false",
        		  	  	     Patterns::Bool(),
...
// the nonlinear poro-viscoelastic solid
    ...
    class Solid
    {
          ...
          protected:
            ...
            CellDataStorage<typename Triangulation<dim>::active_cell_iterator, PointHistory<dim,ADNumberType> > quadrature_point_history;
...
// PerTaskData_ASM stores local contributions
    ...
    void Solid<dim>::system_setup(TrilinosWrappers::MPI::BlockVector &solution_delta_OUT)
    {
        ...
        DoFTools::make_sparsity_pattern (dof_handler_ref, bsp, hanging_node_constraints,
        ...
        TrilinosWrappers::SparsityPattern sp (locally_owned_dofs,
                                              mpi_communicator);
        DoFTools::make_sparsity_pattern (dof_handler_ref, sp, hanging_node_constraints,
                                         false, this_mpi_process);
        sp.compress();
    ...
    class GradientPostprocessor : public DataPostprocessorVector<dim>
    {
      ...
      // Also, solution (displ and pressure) aren't computed correctly, but load seems to be.
    ...
    template <int dim>
    class BrainBudday2017CubeConsolidation
          : public BrainBudday2017BaseCube<dim>
    {
        public:
          BrainBudday2017CubeConsolidation (const Parameters::AllParameters &parameters)
            : BrainBudday2017BaseCube<dim> (parameters)
          {}
    ...
    // We group the definition of the geometry, boundary and loading conditions specific to
    // the examples to reproduce experiments at LTM-FAU into specific classes.

    //@sect4{Base class: Sample geometry}
    template <int dim>
    class BrainRheometerLTMBase
          : public Solid<dim>
    {
        public:
    ...
    //@sect4{Function to define Dirichlet boundary conditions for rotational shear}
    template <int dim>
      ...
      void vector_value_list(const std::vector<Point<dim>>  &points,
                             std::vector<Vector<double>>    &value_list) const
       {
         ...
         for (unsigned int p=0; p<n_points; ++p)
           get_dirichlet_bc_LTMShear<dim>::vector_value (points[p],
                                                         value_list[p]);
       }
    ...
    //@sect4{Derived class: Cyclic rotational shear, only lateral boundaries are drained}
    template <int dim>
    class BrainRheometerLTMShearLateralDrained
          : public BrainRheometerLTMBase<dim>
    {
        public:
          BrainRheometerLTMShearLateralDrained (const Parameters::AllParameters &parameters)
        ...
        private:
                             ...
                             const int                  &direction) const
          {
                ...
                if ( (boundary_id == 2) && (direction == -1) )
                {
                    ...
                    current_angle  = total_angle * (std::sin(2.0*(numbers::PI)*num_cycles*current_time/final_time));
                    previous_angle = total_angle * (std::sin(2.0*(numbers::PI)*num_cycles*(current_time-delta_time)/final_time));
    ...
    //@sect4{Derived class: Cyclic rotational shear, only lateral boundaries are drained}
    template <int dim>
    class BrainRheometerLTMShearRelaxationLateralDrained : public BrainRheometerLTMBase<dim>
    {
        public:
        	BrainRheometerLTMShearRelaxationLateralDrained (const Parameters::AllParameters &parameters) : BrainRheometerLTMBase<dim> (parameters) {}
                ...
                if ((boundary_id == 2) && (direction == -1)) {
                    ...
                    if (current_time <= final_load_time) {
        				...
        				if (current_time > delta_time)
        					previous_angle = ((current_time-delta_time)/final_load_time) * total_angle;
                    	// max magnitude of displ. increment in x-y plane (at max radius)
                    	displ_incr[0] = current_angle - previous_angle;
                    	// previous angle value needed to compute spatial distribution of Dirichlet bcs correctly
                    	displ_incr[1] = previous_angle;
                    	//vertical load
                    	displ_incr[2] = 0;
                    } else
    ...
    template <int dim>
    class BrainRheometerLTMCyclicTensionCompression : public BrainRheometerLTMBase<dim>
    {
        public:
    ...
    //@sect4{Derived class: Cyclic Tension and Compression}
        ...
        {
            public:
        		BrainRheometerLTMCyclicTrapezoidalTensionCompression (const Parameters::AllParameters &parameters) : BrainRheometerLTMBase<dim> (parameters) {}
            ...
            private:
                ...
                virtual std::vector<double> get_dirichlet_load(const types::boundary_id &boundary_id, const int &direction) const
                {
                      ...
                      if ((boundary_id == 2) && (direction == 2)) {
                          ...
                          const double cycle_time   = (final_time-2*num_cycles*hold_time)/(4*num_cycles); //15s
                          const double displ_increment = (delta_time/cycle_time) * final_displ; //0.04mm/s
    ...
    template <int dim>
    class BrainRheometerLTMRelaxationTensionCompression : public BrainRheometerLTMBase<dim>
    {
        public:
       		BrainRheometerLTMRelaxationTensionCompression (const Parameters::AllParameters &parameters) : BrainRheometerLTMBase<dim> (parameters) {}
    ...
    {
        public:
            BrainRheometerLTMBaseQuarter (const Parameters::AllParameters &parameters) : Solid<dim> (parameters) {}
    ...
    //@sect4{Derived class: Cyclic Tension and Compression}
    template <int dim>
    class BrainRheometerLTMCyclicTensionCompressionQuarter : public BrainRheometerLTMBaseQuarter<dim>
    {
        public:
    		BrainRheometerLTMCyclicTensionCompressionQuarter (const Parameters::AllParameters &parameters) : BrainRheometerLTMBaseQuarter<dim> (parameters) {}
   ...
   {
	   public:
		BrainRheometerLTMCyclicCompressionQuarter (const Parameters::AllParameters &parameters) : BrainRheometerLTMBaseQuarter<dim> (parameters) {}
   ...
   //@sect4{Derived class: Cyclic Tension and Compression}
     template <int dim>
     class BrainRheometerLTMCyclicTensionQuarter : public BrainRheometerLTMBaseQuarter<dim>
     {
  	   public:
  		BrainRheometerLTMCyclicTensionQuarter (const Parameters::AllParameters &parameters) : BrainRheometerLTMBaseQuarter<dim> (parameters) {}
    ...
    template <int dim>
    class BrainRheometerLTMCyclicTrapezoidalTensionCompressionQuarter : public BrainRheometerLTMBaseQuarter<dim>
    {
        public:
    		BrainRheometerLTMCyclicTrapezoidalTensionCompressionQuarter (const Parameters::AllParameters &parameters) : BrainRheometerLTMBaseQuarter<dim> (parameters) {}
    ...
    template <int dim>
    class BrainRheometerLTMRelaxationTensionCompressionQuarter : public BrainRheometerLTMBaseQuarter<dim>
    {
        public:
    		BrainRheometerLTMRelaxationTensionCompressionQuarter (const Parameters::AllParameters &parameters) : BrainRheometerLTMBaseQuarter<dim> (parameters) {}
    ...
    // We group the definition of the geometry, boundary and loading conditions specific to
    // the examples to reproduce nanoindentation experiments into specific classes.
    ...
    public:
      ...
      :
      Function<dim> (dim+1),
      indent_dist_current(indent_dist_current),   //max indent dist (in % radius) for this dt
      indent_dist_previous(indent_dist_previous), //max indent dist (in % radius) for previous dt
    ...
    template <int dim>
    class BrainNanoSpherIndentBase
          : public Solid<dim>
    {
        public:
        ...
        private:
          ...
          make_grid()
          {
            ...
            const double max_indent_radius = (1.0-max_indent_dist)*indenter_radius;
              ...
              for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
              {
                if (cell->face(face)->at_boundary() == true)
                {
                  ...
                  else if (cell->face(face)->center()[2] == thickness)  // top
                  {
                      ...
                      const double max_bc_radius_sq = indenter_radius*indenter_radius
                                               - max_indent_radius*max_indent_radius;
                    
                      if ((cell->face(face)->center()[0]*cell->face(face)->center()[0] +
                           cell->face(face)->center()[1]*cell->face(face)->center()[1])
                            <= max_bc_radius_sq * margin )
                          cell->face(face)->set_boundary_id(5); // final loaded surface
                      else
                          cell->face(face)->set_boundary_id(1);
                  }
                      ...
                      cell->face(face)->set_boundary_id(4); //rest of lateral sides
                }
              }
            }
             ...
             if (this->parameters.load_type == "displacement")
             {
                if (this->time->get_current()<=this->parameters.end_load_time)
                {
                    ...
                    VectorTools::interpolate_boundary_values(
                             this->dof_handler_ref,
                             5,
                             get_dirichlet_bc_BrainNanoSpherIndent<dim>(value[0],value[1],value[2]),
                             constraints,
                             this->fe.component_mask(this->z_displacement)                    );
                }
                else
    ...
    {
        public:
          BrainNanoSpherIndentSinusoidalLoad (const Parameters::AllParameters &parameters)
        ...
        private:
                             ...
                             const int                  &direction) const
          {
                ...
                if ( (boundary_id == 5) && (direction == -1) ) //Nanoindentation experiments
                {
                    if (this->parameters.num_cycle_sets > 1)
                        AssertThrow(false, ExcMessage("Problem type not defined. Nanoindentation experiments implemented only for one set of cycles."));
                    ...
                    current_displ  = -1.0 * final_displ/2.0 * (1.0 - std::sin(numbers::PI * (2.0*num_cycles*current_time/final_time + 0.5)));
                    previous_displ = -1.0 * final_displ/2.0 * (1.0 - std::sin(numbers::PI * (2.0*num_cycles*(current_time-delta_time)/final_time + 0.5)));

                    displ_incr[0] = current_displ - previous_displ;  //max indent displ for this dt
    ...
    template <int dim>
    class BrainNanoSpherIndentRampLoad
          : public  BrainNanoSpherIndentBase<dim>
    {
        public:
    ...
    // Examples to reproduce nanoindentation experiments at LTM (FAU) with a flat punch.
    // We group the definition of the geometry, boundary and loading conditions specific to
    // the examples.

    // Base class: Sample geometry for flat punch
    template <int dim> class BrainNanoFlatPunchBase : public Solid<dim>
    {
    	public:
    		BrainNanoFlatPunchBase (const Parameters::AllParameters &parameters) : Solid<dim> (parameters) {}
    	...
    	private:
    		virtual void make_grid()
    		{
    			...
    			// Create a subdivided_hyper_rectangle with two cells along x- and y-direction and one cell in z-direction
    			const std::vector<unsigned int> repetitions = {2, 2, 1};
    			GridGenerator::subdivided_hyper_rectangle(this->triangulation, repetitions, origin, outside);
    			...
    			*/
    			...
    			// obtain 4 cells inside the radius of 9mm on the top surface -> move vertices accordingly
    			for (const auto &cell : this->triangulation.active_cell_iterators()) {
    				for (unsigned int i = 0; i < GeometryInfo<dim>::vertices_per_cell; ++i) {
    					Point<dim> &v = cell->vertex(i);
    		...
    		virtual void define_tracked_vertices(std::vector<Point<dim>> &tracked_vertices)
    		{
    			tracked_vertices[0][0] = 0.0*this->parameters.scale;
    			tracked_vertices[0][1] = 0.0*this->parameters.scale;
    			tracked_vertices[0][2] = 2.0*this->parameters.scale; // TODO: use height instead of 3.5 or even top_center directly
    ...
    template <int dim> class BrainNanoFlatPunchRampLoad : public BrainNanoFlatPunchBase<dim>
	{
		public:
    		BrainNanoFlatPunchRampLoad (const Parameters::AllParameters &parameters) : BrainNanoFlatPunchBase<dim> (parameters) {}
    		...
    		virtual std::vector<double> get_dirichlet_load(const types::boundary_id &boundary_id, const int &direction) const
			{
    			...
    			if ((boundary_id == 100) && (direction == 2)) {
    				...
    				if (current_time <= final_load_time) {
    					...
    					displ_incr[2] = current_displ - previous_displ;
    				} else if (current_time > start_deload_time && current_time <= end_deload_time) {
...
//@sect4{Base class: Quarter Sample geometry for Graz experiments}
    ...
    {
        public:
    		HydroNanoGrazBaseQuarter (const Parameters::AllParameters &parameters) : Solid<dim> (parameters) {}
        ...
        private:
            virtual void make_grid()
            {
            	...
            	// Assign a cylindrical manifold to the geometry
            	const CylindricalManifold<dim> cylinder_3d(2); //cylinder along axis=2 -> z-axis
            	const types::manifold_id cylinder_id = 0;
            	this->triangulation.set_manifold(cylinder_id, cylinder_3d);
            	...
            	for (const auto &cell : this->triangulation.active_cell_iterators()) {
            		if (displ_center.distance(cell->center()) < 5 && cell->center()[2] > (height - 3))
            			cell->set_refine_flag();
            	}
            	...
            	for (const auto &cell : this->triangulation.active_cell_iterators()) {
            		if (displ_center.distance(cell->center()) < 2.1 && displ_center.distance(cell->center()) > 1.9 && cell->center()[2] > (height - 0.15))
            			cell->set_refine_flag();
            	}
    ...
    //@sect4{Derived class: Tension and Compression Relaxation}
    template <int dim>
    class HydroNanoGrazRelaxationCompressionQuarter : public HydroNanoGrazBaseQuarter<dim>
    {
        public:
    	HydroNanoGrazRelaxationCompressionQuarter (const Parameters::AllParameters &parameters) : HydroNanoGrazBaseQuarter<dim> (parameters) {}
    ...
    {
    	public:
    		get_dirichlet_bc_HydroNanoGrazSpherIndent (const double d_c, const double d_p, const double r_i)
    		:
    		Function<dim> (dim+1),
			d_c(d_c),   // current indentation depth
			d_p(d_p), 	// previous indentation depth
			r_i(r_i)    // indenter radius
			{}

    		void vector_value(const Point<dim> &pt, Vector<double> &values) const
    		{
    		...
    		void vector_value_list(const std::vector<Point<dim>> &points, std::vector<Vector<double>> &value_list) const
    		{
    			...
    			for (unsigned int p=0; p<n_points; ++p)
    				get_dirichlet_bc_HydroNanoGrazSpherIndent<dim>::vector_value (points[p], value_list[p]);
    		}
	...
	{
		public:
			HydroNanoGrazBaseQuarterSphere (const Parameters::AllParameters &parameters) : Solid<dim> (parameters) {}
		...
		private:
			virtual void make_grid()
			{
				...
				// maximal indentation radius based on indenter radius and maximal indentation depth
				const double r_max = std::sqrt((2.0*r_i*std::abs(d_max)-std::abs(d_max)*std::abs(d_max)));
				...
				for (const auto &cell : this->triangulation.active_cell_iterators()) {
					if (displ_center.distance(cell->center()) < 4 && cell->center()[2] > (height - 2.5))
						cell->set_refine_flag();
				}
				...
				for (const auto &cell : this->triangulation.active_cell_iterators()) {
					if (displ_center.distance(cell->center()) < 2.8 && cell->center()[2] > (height - 1.2))
						cell->set_refine_flag();
				}
	...
	//@sect4{Derived class: Tension and Compression Relaxation}
	template <int dim>
	class HydroNanoGrazRelaxationCompressionQuarterSphere : public HydroNanoGrazBaseQuarterSphere<dim>
	{
		public:
		HydroNanoGrazRelaxationCompressionQuarterSphere (const Parameters::AllParameters &parameters) : HydroNanoGrazBaseQuarterSphere<dim> (parameters) {}
  ...
  try
    {
      ...
      {
        BrainRheometerLTMShearLateralDrained<3> solid_3d(parameters);
        solid_3d.run();
      }
      else if (parameters.geom_type == "brain_rheometer_shear_relaxation_lateral_drained")
      {
        BrainRheometerLTMShearRelaxationLateralDrained<3> solid_3d(parameters);
        solid_3d.run();
      }
      else if (parameters.geom_type == "brain_rheometer_cyclic_tension_compression")
      {
      ...
      {
    	BrainRheometerLTMCyclicTrapezoidalTensionCompression<3> solid_3d(parameters);
    	solid_3d.run();
      }
      else if (parameters.geom_type == "brain_rheometer_relaxation_tension_compression")
      {
    	BrainRheometerLTMRelaxationTensionCompression<3> solid_3d(parameters);
    	solid_3d.run();
      }
      ...
      {
        BrainRheometerLTMCyclicCompressionQuarter<3> solid_3d(parameters);
        solid_3d.run();
      }
      else if (parameters.geom_type == "brain_rheometer_cyclic_tension_quarter")
      {
        BrainRheometerLTMCyclicTensionQuarter<3> solid_3d(parameters);
        solid_3d.run();
      }
      else if (parameters.geom_type == "brain_rheometer_cyclic_trapezoidal_tension_compression_quarter")
      {
        BrainRheometerLTMCyclicTrapezoidalTensionCompressionQuarter<3> solid_3d(parameters);
        solid_3d.run();
      }
      else if (parameters.geom_type == "brain_rheometer_relaxation_tension_compression_quarter")
      {
      ...
      else if (parameters.geom_type == "brain_nanoindentation_ramp")
      {
        BrainNanoSpherIndentRampLoad<3> solid_3d(parameters);
        solid_3d.run();
      }
      else if (parameters.geom_type == "brain_nanoindentation_flat_ramp")
      {
    	BrainNanoFlatPunchRampLoad<3> solid_3d(parameters);
    	solid_3d.run();
      }
      else if (parameters.geom_type == "hydro_nano_graz_compression_relax")
      {
